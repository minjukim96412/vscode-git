...
Promise
    ...
    - 그래서 비동기작업(XHRHttpRequest)이나 타이머작업(setTimeout, setInterval)을
      수행할때 다음 줄의 코드들이 먼저 수행되는 고질적 문제가 있다.
    - 콜백헬(callback hell) : 코드 실행순서를 보장하기 위해서 콜백함수를 사용하게
        되는데 순서를 보장할 코드가 늘어나면 콜백함수의 수가 늘어나서 코드의 복잡
        도가 높아지는 문제가 있다. 이를 콜백헬이라 함
    - 이전 코드의 수행결과(성공 또는 실패)를 다음코드에서 보장받아(약속받아)
      수행하게 하기위해 ES6에 Promise가 추가됨
...
//비동기 처리가 완료되기 전에 아래코드가 수행되어서 todos가 undefined가 됨
console.log(todos);
...
console.log(rejected); //reject 상태
