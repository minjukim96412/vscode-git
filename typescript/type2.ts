...
const bird1: Bird = {
    name: '독수리', legCnt: 2, hasWing: true
};
//const bird2: Bird = {name: '독수리', legCnt: 2};// error 옵셔널(?)이 있어야 가능

//객체 프라퍼티명으로 숫자를 사용
interface Student1 {
    [key: number]: string;
}
const student1: Student1 = {1:'홍길동', 2:'강감찬'};
...
const init1: Init1 = {name: '홍길동', age: 30};
...
const venus: number = Planet.VENUS;
const mecury: number = Planet.MERCURY;
...
//클래스
// - 타입스크립트에서는 클래스의 프라퍼티들을 미리 정의해 줘야 함
...
lengthOnly('123');//문자열은 length프라퍼티를 가지고 있다.
...
console.log(lengthOnly2("123"));//3
console.log(lengthOnly2(123));//123

//keyof : 객체의 프라퍼티들의 키들을 추출해서 문자열 유니언 타입으로 변환
