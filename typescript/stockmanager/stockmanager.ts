...
const updateShopList = () => {
    ...
    shopList.forEach((shop: Shop, index: number) => {
    ...
    $("#shopBody").on("click", "tr", () => {
        ...
        const shopId: number = parseInt($().find(".shno").text());
        updateStockList(shopId);
    });

    // 재고 등록 버튼 클릭 이벤트
    $('#stwriteBtn').on('click', () => {
        const selectedShopId: number = parseInt($("#shopBody tr.selected").find(".shno").text()); // 선택된 매장의 ID 가져오기
        if (selectedShopId) {
    ...
    $("#stokBody").on("click", ".deleteBtn", (event) => {
        ...
        removeStok(stno);
        const selectedShopId: number = parseInt($("#shopBody tr.selected").find(".shno").text());
    ...
    $("#stokBody").on("click", ".stamt", (event) => {
        ...
        const stno: number = parseInt($(event.currentTarget).closest("tr").find("td:first").text());
        const selectedShopId = parseInt($("#shopBody tr.selected").find(".shno").text());
        
        // 해당 stno 값으로 재고 항목 찾기
        const stockList: Stock[] = getStockList();
...
// 매장 등록
const writeShop = () => {
    const shopArr: Shop[] = JSON.parse(localStorage.getItem('shopList') !);
    shopArr.push(new Shop(getNextShopSeq(), $('#shname').val() as string, 0));
...
const getTotalStockAmount = (shopId: number) => {
    const stockList: Stock[] = getStockList(); // 모든 재고 목록 가져오기
    let totalAmount = 0;
...
// 매장명 수정
const updateShop = (index: number, newName: string) => {
...
// 재고 목록 업데이트
const updateStockList = (shopId: number) => {
    const stockList = getStockList().filter((stock: Stock) => stock.shno == shopId);
...
// 재고 등록
const writeStock = (selectedShopId: number) => {
    const shopList = getShopList();
    const stockArr: Stock[] = JSON.parse(localStorage.getItem('stokList') !) || [];
    const strgdate = new Date();
    const stno = getNextStockSeq(); // 고유한 stno를 하나 생성
    const stamt = parseInt($('#stamt').val() as string); // 사용자가 입력한 재고 수량 가져오기

    shopList.forEach((shop: Shop) => {
        let totalStockAmt = 0; // 각 매장의 총 재고 수량을 계산하기 위한 변수 초기화
        if (shop.shno == selectedShopId) {
            // 선택한 매장에 대해서는 사용자가 입력한 재고 수량으로 등록
            stockArr.push(new Stock(stno, $('#stname').val() as string, stamt, $('#stindate').val() as string, strgdate.toLocaleString('ja-JP'), shop.shno));
            totalStockAmt += stamt; // 등록된 재고 수량을 총 재고 수량에 추가
        } else {
            // 선택하지 않은 매장에 대해서는 재고 수량을 0으로 설정하여 등록
            stockArr.push(new Stock(stno, $('#stname').val() as string, 0, $('#stindate').val() as string, strgdate.toLocaleString('ja-JP'), shop.shno));
        }
...
const popStokUpdate = (stno: number) => {
    Swal.fire({
        title: "재고명을 입력해주세요",
        input: "text",
        inputAttributes: {
...
// 재고 삭제
const removeStok = (stno: number) => {
    let stockList = getStockList();
    stockList = stockList.filter((stock: Stock) => stock.stno !== stno);
    localStorage.setItem("stockList", JSON.stringify(stockList));
};

// 재고수량변경
const popStokAmtUpdate = (index: number) => {
    Swal.fire({
        title: "재고 수량을 입력해주세요",
        input: "number",
        inputAttributes: {
...
const updateStockAmt = (index: number, newAmt: number) => {
...
// 재고 수정
const updateStock = (stno: number, newName: string) => {
    const stockList = getStockList();
    stockList.forEach((stok: Stock) => {
        if (stok.stno === stno) {
            stok.stname = newName;
        }
    });
